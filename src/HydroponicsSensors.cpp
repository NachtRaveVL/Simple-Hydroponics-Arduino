...
Hydroponics_UnitsCategory defaultMeasureCategoryForSensorType(Hydroponics_SensorType sensorType, uint8_t measurementRow)
{
    switch (sensorType) {
        case Hydroponics_SensorType_PotentialHydrogen:
...
void HydroponicsBinarySensor::setMeasurementUnits(Hydroponics_UnitsType measurementUnits, uint8_t measurementRow)
{ ; }

Hydroponics_UnitsType HydroponicsBinarySensor::getMeasurementUnits(uint8_t measurementRow) const
{
...
void HydroponicsAnalogSensor::setMeasurementUnits(Hydroponics_UnitsType measurementUnits, uint8_t measurementRow)
{
...
Hydroponics_UnitsType HydroponicsAnalogSensor::getMeasurementUnits(uint8_t measurementRow) const
{
...
{
    HYDRUINO_SOFT_ASSERT(_dht, SFP(HStr_Err_AllocationFailure));
...
void HydroponicsDHTTempHumiditySensor::setMeasurementUnits(Hydroponics_UnitsType measurementUnits, uint8_t measurementRow)
{
...
Hydroponics_UnitsType HydroponicsDHTTempHumiditySensor::getMeasurementUnits(uint8_t measurementRow) const
{
...
{
    HYDRUINO_SOFT_ASSERT(_dt, SFP(HStr_Err_AllocationFailure));
    ...
    if (isValidPin(_inputPin) && _oneWire && _dt) {
        ...
        HYDRUINO_SOFT_ASSERT(_dt->getResolution() == _inputBitRes, SFP(HStr_Err_OperationFailure));
    ...
    if (_isTakingMeasure && _dt) {
        if (getHydroponicsInstance()->tryGetPinLock(_inputPin, 5)) {
            if (_dt->requestTemperaturesByAddress(_wireDevAddress)) {
                ...
                HYDRUINO_SOFT_ASSERT(!deviceDisconnected, SFP(HStr_Err_MeasurementFailure)); // device disconnected
...
void HydroponicsDSTemperatureSensor::setMeasurementUnits(Hydroponics_UnitsType measurementUnits, uint8_t measurementRow)
{
    if (_measurementUnits != measurementUnits) {
        _measurementUnits = measurementUnits;
...
Hydroponics_UnitsType HydroponicsDSTemperatureSensor::getMeasurementUnits(uint8_t measurementRow) const
{
    return _measurementUnits;
}
...
void HydroponicsSensorData::toJSONObject(JsonObject &objectOut) const
{
    ...
    if (isValidPin(inputPin)) { objectOut[SFP(HStr_Key_InputPin)] = inputPin; }
...
void HydroponicsSensorData::fromJSONObject(JsonObjectConst &objectIn)
{
    ...
    inputPin = objectIn[SFP(HStr_Key_InputPin)] | inputPin;
...
void HydroponicsAnalogSensorData::toJSONObject(JsonObject &objectOut) const
{
    ...
    if (inputBitRes != 8) { objectOut[SFP(HStr_Key_InputBitRes)] = inputBitRes; }
    if (inputInversion != false) { objectOut[SFP(HStr_Key_InputInversion)] = inputInversion; }
    if (measurementUnits != Hydroponics_UnitsType_Undefined) { objectOut[SFP(HStr_Key_MeasurementUnits)] = unitsTypeToSymbol(measurementUnits); }
}
...
void HydroponicsAnalogSensorData::fromJSONObject(JsonObjectConst &objectIn)
{
    ...
    inputBitRes = objectIn[SFP(HStr_Key_InputBitRes)] | inputBitRes;
...
void HydroponicsDigitalSensorData::toJSONObject(JsonObject &objectOut) const
{
    ...
    if (inputBitRes != 9) { objectOut[SFP(HStr_Key_InputBitRes)] = inputBitRes; }
...
void HydroponicsDHTTempHumiditySensorData::toJSONObject(JsonObject &objectOut) const
{
    ...
    if (dhtType != DHT12) { objectOut[SFP(HStr_Key_DHTType)] = dhtType; }
...
void HydroponicsDSTemperatureSensorData::toJSONObject(JsonObject &objectOut) const
{
    ...
    if (isValidPin(pullupPin)) { objectOut[SFP(HStr_Key_PullupPin)] = pullupPin; }
    if (measurementUnits != Hydroponics_UnitsType_Undefined) { objectOut[SFP(HStr_Key_MeasurementUnits)] = unitsTypeToSymbol(measurementUnits); }
}
...
void HydroponicsDSTemperatureSensorData::fromJSONObject(JsonObjectConst &objectIn)
{
    ...
    pullupPin = objectIn[SFP(HStr_Key_PullupPin)] | pullupPin;
    measurementUnits = unitsTypeFromSymbol(objectIn[SFP(HStr_Key_MeasurementUnits)]);
}
