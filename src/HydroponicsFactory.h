...
public:
    ...
    SharedPtr<HydroponicsRelayActuator> addGrowLightsRelay(uint8_t outputPin);                     // Digital output pin this actuator sits on
    ...
    SharedPtr<HydroponicsPumpRelayActuator> addWaterPumpRelay(uint8_t outputPin);                  // Digital output pin this actuator sits on
    ...
    SharedPtr<HydroponicsRelayActuator> addWaterHeaterRelay(uint8_t outputPin);                    // Digital output pin this actuator sits on
    ...
    SharedPtr<HydroponicsRelayActuator> addWaterSprayerRelay(uint8_t outputPin);                   // Digital output pin this actuator sits on
    ...
    SharedPtr<HydroponicsRelayActuator> addWaterAeratorRelay(uint8_t outputPin);                   // Digital output pin this actuator sits on
    ...
    SharedPtr<HydroponicsRelayActuator> addFanExhaustRelay(uint8_t outputPin);                     // Digital output pin this actuator sits on
    ...
    SharedPtr<HydroponicsPWMActuator> addAnalogPWMFanExhaust(uint8_t outputPin,                    // PWM output pin this actuator sits on
    ...
    SharedPtr<HydroponicsPumpRelayActuator> addPeristalticPumpRelay(uint8_t outputPin);            // Digital output pin this actuator sits on
    ...
    SharedPtr<HydroponicsBinarySensor> addLevelIndicator(uint8_t inputPin);                        // Digital input pin this sensor sits on (can make interruptable)
    ...
    SharedPtr<HydroponicsAnalogSensor> addAnalogPhMeter(uint8_t inputPin,                          // Analog input pin this sensor sits on
    ...
    SharedPtr<HydroponicsAnalogSensor> addAnalogTDSElectrode(uint8_t inputPin,                     // Analog input pin this sensor sits on
    ...
    SharedPtr<HydroponicsAnalogSensor> addAnalogTemperatureSensor(uint8_t inputPin,                // Analog input pin this sensor sits on
                                                                   uint8_t inputBitRes = 10);       // ADC bit resolution to use
    // Adds a new analog CO2 sensor to the system using the given parameters.
    // CO2 sensors can be used to ensure proper CO2 levels.
    SharedPtr<HydroponicsAnalogSensor> addAnalogCO2Sensor(uint8_t inputPin,                        // Analog input pin this sensor sits on
                                                           uint8_t inputBitRes = 10);               // ADC bit resolution to use
    ...
    SharedPtr<HydroponicsAnalogSensor> addAnalogMoistureSensor(uint8_t inputPin,                   // Analog input pin this sensor sits on
                                                                uint8_t inputBitRes = 10);          // ADC bit resolution to use
    ...
    SharedPtr<HydroponicsAnalogSensor> addAnalogPWMPumpFlowSensor(uint8_t inputPin,                // Analog input pin this sensor sits on
                                                                    uint8_t inputBitRes = 10);      // ADC bit resolution to use
    ...
    SharedPtr<HydroponicsAnalogSensor> addAnalogWaterHeightMeter(uint8_t inputPin,                 // Analog input pin this sensor sits on
                                                                  uint8_t inputBitRes = 10);        // ADC bit resolution to use
    ...
    SharedPtr<HydroponicsAnalogSensor> addUltrasonicDistanceSensor(uint8_t inputPin,               // Analog input pin this sensor sits on
                                                                    uint8_t inputBitRes = 10);      // ADC bit resolution to use
    ...
    SharedPtr<HydroponicsAnalogSensor> addPowerUsageMeter(uint8_t inputPin,                        // Analog input pin this sensor sits on
    ...
    SharedPtr<HydroponicsDHTTempHumiditySensor> addDHTTempHumiditySensor(uint8_t inputPin,         // OneWire digital input pin this sensor sits on
    ...
    SharedPtr<HydroponicsDSTemperatureSensor> addDSTemperatureSensor(uint8_t inputPin,             // OneWire digital input pin this sensor sits on
                                                                      uint8_t inputBitRes = 9,      // Sensor ADC bit resolution to use
                                                                      uint8_t pullupPin = -1);      // Strong pullup pin (if used, else -1)
