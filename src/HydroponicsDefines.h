...
enum Hydroponics_SubstrateType {
    ...
    Hydroponics_SubstrateType_Dirt,                 // Plain 'ol dirt
...
enum Hydroponics_RailType {
    Hydroponics_RailType_ACPower,                   // AC-based power rail, for larger devices
    Hydroponics_RailType_DCPower,                   // DC-based power rail, for smaller devices
...
// Indicator Position
// Common indicator extents. Specifies relative positioning.
enum Hydroponics_IndicatorPosition {
    Hydroponics_IndicatorPosition_Low,              // Indicator position low
    Hydroponics_IndicatorPosition_Mid,              // Indicator position middle
    Hydroponics_IndicatorPosition_High,             // Indicator position high

    Hydroponics_IndicatorPosition_Count,            // Internal use only
    Hydroponics_IndicatorPosition_Undefined = -1    // Internal use only
};

// Trigger Status
// Common trigger statuses. Specifies enablement and tripped state.
enum Hydroponics_TriggerState {
    Hydroponics_TriggerState_Disabled,              // Triggers disabled
...
struct HydroponicsIdentity {
    ...
    enum {
        Actuator,       // A pump, valve, or other controllable thing
        Sensor,         // A probe, meter, or other measuring device
        Crop,           // A planter, pot, or other plant container
        Reservoir,      // A fluid reservoir, or other special solution
        Rail,           // A power rail, or other isolated power source
        Unknown = -1    // Unknown type
    } type;             // Object type
    union {
        Hydroponics_ActuatorType actuatorType;
        Hydroponics_SensorType sensorType;
        Hydroponics_CropType cropType;
        Hydroponics_ReservoirType reservoirType;
        Hydroponics_RailType railType;
    } as;               // Which type
    Hydroponics_PositionIndex posIndex;
    char key[HYDRUINO_NAME_MAXSIZE];

    inline bool isActuator() const { return type == Actuator; }
    inline bool isSensor() const { return type == Sensor; }
    inline bool isCrop() const { return type == Crop; }
    inline bool isReservoir() const { return type == Reservoir; }
    inline bool isRail() const { return type == Rail; }
    inline bool isUnknown() const { return type == Unknown; }
    inline Hydroponics_ActuatorType getActuator() const { return isActuator() ? as.actuatorType : Hydroponics_ActuatorType_Undefined; }
    inline Hydroponics_SensorType getSensor() const { return isSensor() ? as.sensorType : Hydroponics_SensorType_Undefined; }
    inline Hydroponics_CropType getCrop() const { return isCrop() ? as.cropType : Hydroponics_CropType_Undefined; }
    inline Hydroponics_ReservoirType getReservoir() const { return isReservoir() ? as.reservoirType : Hydroponics_ReservoirType_Undefined; }
    inline Hydroponics_RailType getRail() const { return isRail() ? as.railType : Hydroponics_RailType_Undefined; }
};

// Hydroponic Object Base
// A simple base class for referring to objects in the Hydroponics system.
// Basically a paper's please base object.
class HydroponicsObject {
public:
    HydroponicsObject(HydroponicsIdentity id);
    virtual ~HydroponicsObject();

    virtual void update();

    const HydroponicsIdentity getId() const;
    const String getKey() const;

protected:
    HydroponicsIdentity _id;
};
