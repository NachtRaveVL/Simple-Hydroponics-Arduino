...
//
// AdafruitDrawable<Adafruit_PCD8544>
//

void AdafruitDrawable<Adafruit_PCD8544>::transaction(bool isStarting, bool redrawNeeded) {
    if(!isStarting && redrawNeeded) {
        reinterpret_cast<Adafruit_PCD8544*>(graphics)->display();
    }
}

void AdafruitDrawable<Adafruit_PCD8544>::internalDrawText(const Coord &where, const void *font, int mag, const char *sz) {
...
void AdafruitDrawable<Adafruit_PCD8544>::drawBitmap(const Coord &where, const DrawableIcon *icon, bool selected) {
...
void AdafruitDrawable<Adafruit_PCD8544>::drawXBitmap(const Coord &where, const Coord &size, const uint8_t *data) {
...
void AdafruitDrawable<Adafruit_PCD8544>::drawBox(const Coord &where, const Coord &size, bool filled) {
    if(filled) {
...
void AdafruitDrawable<Adafruit_PCD8544>::drawCircle(const Coord& where, int radius, bool filled) {
    if(filled) {
...
void AdafruitDrawable<Adafruit_PCD8544>::drawPolygon(const Coord points[], int numPoints, bool filled) {
    if(numPoints == 2) {
...
Coord AdafruitDrawable<Adafruit_PCD8544>::internalTextExtents(const void *f, int mag, const char *text, int *baseline) {
...
void AdafruitDrawable<Adafruit_PCD8544>::drawPixel(uint16_t x, uint16_t y) {
    graphics->writePixel(x, y, drawColor);
}

UnicodeFontHandler *AdafruitDrawable<Adafruit_PCD8544>::createFontHandler() {
