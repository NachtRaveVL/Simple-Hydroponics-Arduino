...
template <class T>
HydroOverviewAdaTFT<T>::HydroOverviewAdaTFT(HydroDisplayAdafruitGFX<T> *display)
    : HydroOverview(display), _gfx(display->getGfx()), _drawable(display->getDrawable()),
      _lastTime((uint32_t)0)
{ ; }

template <class T>
HydroOverviewAdaTFT<T>::~HydroOverviewAdaTFT()
{ ; }

template <class T>
void HydroOverviewAdaTFT<T>::drawBackground(Coord pt, Coord sz, Pair<uint16_t, uint16_t> &screenSize)
{
    uint16_t &width(screenSize.first); uint16_t &height(screenSize.second);
    pt.x = constrain(pt.x, 0, width);
    sz.x = constrain(sz.x, 0, width - pt.x);
...
template <class T>
void HydroOverviewAdaTFT<T>::renderOverview(bool isLandscape, Pair<uint16_t, uint16_t> screenSize)
{
    // todo
}
