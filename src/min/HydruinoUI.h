...
public:
    ...
    void allocateTFTTouchControl();                                         // Allocates/pulls-into-build any TFTTouch controls (using TFTe_SPI), must be called after display allocation
    ...
    void addSerialRemote(UARTDeviceSetup rcSetup = UARTDeviceSetup());      // Adds/pulls-into-built remote control by Serial or Bluetooth AT
    void addSimhubRemote(UARTDeviceSetup rcSetup = UARTDeviceSetup());      // Adds/pulls-into-built remote control by Simhub serial connector, requires UART setup
    void addWiFiRemote(uint16_t rcServerPort = HYDRO_UI_REMOTESERVER_PORT); // Adds/pulls-into-built remote control by WiFi, requires enabled WiFi
    void addEthernetRemote(uint16_t rcServerPort = HYDRO_UI_REMOTESERVER_PORT); // Adds/pulls-into-built remote control by Ethernet, requires enabled Ethernet
